...
Check project 1 for the Sugartown repo:
```bash
...
Based on the branch map, board status, and the MEMORY.md context you have about this project:
...
- Does the board suggest a different priority than what the git state shows?

Keep this section short — 3–5 sentences max. Do not invent work. Only suggest what is visible from the git state, board status, and project memory.
