...
- [ ] Add `Docs/Story Template.stories.tsx` at `apps/storybook/.storybook/stories/` — the `Docs/` sidebar section already has Welcome, Introduction, Component Contracts, Contributing; this adds a new story explaining the 14-section structure, coverage model (autodocs vs Guidelines), and import conventions for the docs template system — layer: Storybook
...
**Phase 2 — Docs/ Story Template story** (Docs/ already has: Welcome, Introduction, Component Contracts, Contributing)
- `Docs/Story Template.stories.tsx` — explains 14-section model, coverage map (autodocs vs Guidelines), import pattern for helpers/docs.tsx
- Optionally: a live reference section within the same story showing every helper component rendered with the correct Pink Moon visual language
...
- [ ] `pnpm storybook` starts without errors; `Docs/Story Template` story visible alongside existing Docs/ entries (Welcome, Introduction, Component Contracts, Contributing)
