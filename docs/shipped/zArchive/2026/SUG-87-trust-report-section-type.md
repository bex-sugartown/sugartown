...
- [x] **DS primitives** — `StatTile` + `DataTable` + `KindBadge` components, CSS modules, Storybook stories, token additions
- [x] **Sanity schema** — `trustReportSection` object in `schemas/sections/`; single `reportType` select field (`recent-releases` | `design-system-stats`); register in doc types
- [x] **GROQ projection** — add `_type == "trustReportSection" => { reportType }` to all slug queries
- [x] **Renderer** — `TrustReportSection.jsx` in `apps/web/src/components/`; switch on `reportType`; pull from `stats.json`; two sub-renderers consuming `StatTile` and `DataTable`
- [x] **CSS** — component CSS, token-first, both dark and light theme
- [x] **Section builder wiring** — add to `PageSections.jsx` switch + all in-scope doc type schemas
...
- [x] `StatTile` component exists in `apps/web/src/design-system/components/stat-tile/` with Storybook story
- [x] `DataTable` component exists in `apps/web/src/design-system/components/data-table/` with Storybook story
- [x] `trustReportSection` schema exists, registered in `index.ts`, available in section builder for page/article/caseStudy/node
- [x] Selecting `recent-releases` renders N most recent releases via `DataTable` from `stats.release.latestN`
- [x] Selecting `design-system-stats` renders DS health data via `StatTile` grid from `stats.json`
- [x] No editorial content fields — only the `reportType` selector in Studio
- [x] Both variants wired to `/platform` page with real data
- [x] `pnpm validate:tokens` — 0 errors
- [x] `pnpm validate:tokens --strict-colors` — 0 errors
- [x] Storybook stories for both variants and both primitives exist and render without errors
- [x] Phase 0 mock approved before any implementation code is written
