...
- [ ] **SegmentedControl / IndexGroup + IndexCell** ⚠️ DECISION NEEDED — `IndexGroup` + `IndexCell` implement what every other DS calls a segmented control (adjacent square cells, selected/active/inactive states). `AlphaFilter` is a confirmed Pattern composed from these two primitives (correct — not a dupe). Decision required: (a) rename `IndexGroup` → `SegmentedControl` and `IndexCell` → `SegmentItem` at DS level, update `AlphaFilter` + `SiteGraphPage` callers, move story to `Components/SegmentedControl`; or (b) keep current names and file `SegmentedControl` as a synonym/alias in the audit. Note: `IndexCell` has text content (letters, "All") so is NOT a dupe of `IconButton` (icon-only). Token unification (0px radius) is already correct in `IndexCell.module.css` — verify `IconButton` matches. Blocked until decision.
