...
| CTL-031 | `validate:governance` — schema and closed-world referential integrity over `governance/source/`, overdue `nextRead` on `cadence: dated` records, a scan for governance data authored outside `governance/source/`, and two-way correspondence between `probe` records and the liveness harness gate list (obtained by spawning `validate-enforcement-liveness.js --list-gates`, never imported: `main()` runs at module scope). Reads the index, anchored on the repo root | enforced-by-code | `validate:governance` | `.husky/pre-commit` blocks the commit; `ci.yml` step *Validate governance* → `ci-failure-alert.yml` (CTL-013) | continuous | `git commit --no-verify`; a `[skip ci]` commit runs no CI; merge commits run no pre-commit hook (only `.husky/pre-commit` exists); Netlify publishes from `main` regardless of CI (CTL-020) and does not run this check until Phase 4. Proves correspondence and shape, never truth: a schema-valid but factually wrong value passes. **Scan coverage:** at least 300 files across `apps/web/src/`, `docs/ai/agentic-caucus/`, `scripts/`, `packages/` — read the count from `pnpm validate:governance` rather than from this cell, because it moves with ordinary churn. A register or coverage table under `docs/briefs/`, `docs/backlog/`, `docs/shipped/`, `docs/reviews/` or `.claude/` is invisible to it, as is a coverage table with any column reworded. Hardcoded governance values in JSX are out of scope by design → CTL-033 (Phase 4). **The two-way check proves correspondence, not coverage:** a gate with neither a probe nor a record satisfies both directions vacuously, and a record asserts a probe exists rather than that it ran (`validate:epic-docs` and `chromatic.sh reachability` legitimately skip). Gate-set coverage stays with `validate:controls` check 3. Probed three times in `PROBES` — the scan, the `--list-gates` contract, and a fixture asserting both orphan directions — while schema, referential integrity and overdue detection remain unprobed. `derivation` accuracy is CTL-035. **Deferred, with owners:** `claim.command` runner resolution → SUG-274; crosswalk layer completeness → Phase 3; `claim.statsKey` resolution → undated, needs a date before this row is next read |
