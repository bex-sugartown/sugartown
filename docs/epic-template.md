...
- [ ] **Enforcement liveness — declared is not effective** (blocking for any epic that audits, adds, or relies on a rule, gate, or validator) — for every rule this epic claims is in force, prove it *fires*; do not confirm it *exists*. Config presence, a wired hook, and a passing validator are three different things from an enforced rule. Proof is a deliberate violation that fails then reverts — or where impractical, the resolved config read back from the tool itself (`eslint --print-config`). "The rule is declared in X" is not evidence. This failed five ways across 2026-05→07: `boundaries.js`'s four rules were configured and never fired, the Chromatic job died before running, CI was red for three months, `sugartown_check_boundary` answered from intent rather than behaviour, and `validate:validators` — built expressly to catch silent enforcement decay — passed throughout, because it verifies a validator is wired and cannot verify its result is read (SUG-254, SUG-255).
- [ ] **Control register rows** (blocking for any epic that adds or changes a gate, validator, deploy path, or published claim) — every new control has a row in `docs/ai/agentic-caucus/control-register.md` naming its probe, its reader, and its next-read date, committed in the same epic. A gate added without a probe is how the probe set falls behind the gate set: 6 of 12 `validate:*` scripts reached 2026-07 unprobed this way. See `docs/conventions/verification-review.md`.
