...
| CTL-031 | `validate:governance` — schema and closed-world referential integrity over `governance/source/`, overdue `nextRead` on `cadence: dated` records, and a scan for governance data authored outside `governance/source/`. Reads the index, anchored on the repo root | enforced-by-code | `validate:governance` | `.husky/pre-commit` blocks the commit; `ci.yml` step *Validate governance* → `ci-failure-alert.yml` (CTL-013) | continuous | `git commit --no-verify`; a `[skip ci]` commit runs no CI; merge commits run no pre-commit hook (only `.husky/pre-commit` exists); Netlify publishes from `main` regardless of CI (CTL-020) and does not run this check until Phase 4. Proves correspondence and shape, never truth: a schema-valid but factually wrong value passes. **Scan coverage, measured 2026-08-06 by `pnpm validate:governance` over 396 files** in `apps/web/src/`, `docs/ai/agentic-caucus/`, `scripts/`, `packages/` — so a register or coverage table pasted under `docs/briefs/`, `docs/backlog/`, `docs/shipped/`, `docs/reviews/` or `.claude/` is invisible to it. A coverage table with any column reworded, or coverage rows pasted without their header, escapes all four patterns. Hardcoded governance values in JSX are out of scope by design → CTL-033 (Phase 4). **Deferred, with owners:** two-way probe↔harness check → SUG-273; `claim.command` runner resolution → SUG-274; crosswalk layer completeness → Phase 3; `claim.statsKey` resolution → undated, needs a date before this row is next read |
