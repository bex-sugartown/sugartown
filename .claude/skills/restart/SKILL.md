---
name: restart
description: Check which Sugartown dev servers are running and restart any that are down.
---
...
| Contentful POC    | 3000 | `pnpm --filter contentful-poc dev` (from repo root) |

## Procedure

1. For each port (5173, 3333, 6006, 3000), check if a process is listening: `lsof -ti:{port}`
2. Report status of all four services (running / stopped)
...
- If the user says just `/restart` with no arguments, check all four and restart any that are down
- If the user says `/restart 5173` or `/restart web`, only restart that one service
- Accepted aliases: `web` = 5173, `studio` = 3333, `storybook` or `sb` = 6006, `contentful` or `poc` = 3000
