...
4. **`extendedDefinitions` nested-sense migration:** NOT in scope. The handoff's nested `ol` sense/subsense structure would require a PT → custom-array schema migration. `extendedDefinition` stays Portable Text; ordered lists within PT already render. If true Merriam-Webster sense structures are needed later, that is its own epic with its own schema proposal.
...
- **`extendedDefinitions` nested sense/subsense schema migration** — handoff Open Item 3; stays Portable Text. Separate epic if ever needed.
