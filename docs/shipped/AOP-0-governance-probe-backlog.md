...
**Linear Issue:** **None, by decision (2026-08-10).** Not an oversight and not a pending task. This epic closed before an ID was ever spent on it, so an ID would have been created already Done — occupying a slot in a workspace at its issue limit while buying no queue visibility. The three remaining reusable IDs went to live work instead (`SUG-281`, `SUG-269`). Its shipped work is recorded in `CHANGELOG.md`'s `[Unreleased]` buffer and in `control-register.md` rows CTL-011 and CTL-016, which is where a reader would actually look. Filename keeps the `AOP-` prefix rather than taking a SUG number, on the same precedent as the legacy `EPIC-NNNN` docs in this directory.
...
**Status:** **Closed 2026-08-09 — 2 of 6 probed, 4 filed with reasons.** Harness went 21 → 23 gates proven live, 0 inert. The other four were not skipped: three are structurally unprobeable and moved to `SUG-269`; `pnpm test:smoke` needs a cost decision, recorded there too. See §Findings
