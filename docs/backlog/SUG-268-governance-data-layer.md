...
**Status:** In Progress — **Phase 1 complete and merged** (2026-08-05, CI 31026674863 green). **Phase 2 complete on local `main` 2026-08-07** — all 13 scope items ticked, 7 `Ph2` commits from `6424b7fe` to `ca2a73ca` plus the CTL-031/CTL-015 amendment in `c6a4e836`. **Merged and green: CI run 31169229182, `success`, 2026-08-07** on `daac000a`. Locally `pnpm validate:enforcement-liveness` reports 19 gates live, 0 inert, 2 skipped (`validate:epic-docs` has no local `LINEAR_API_KEY`; `chromatic.sh` needs `.env` absent) — both are covered by the CI run. The first push failed as run 31168947110 on `Validate epic docs`: SUG-276 had been filed in Linear with no backlog doc, and that gate skips locally for want of the API key, so pre-commit could not catch it. Fixed in `daac000a`. Phases 3–4 outstanding.
