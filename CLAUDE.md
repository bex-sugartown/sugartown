...
1b. **Route smoke tests** — `pnpm test:smoke` passes locally, or the CI job is green after push. Five Playwright specs prove the app actually renders end-to-end (not just builds cleanly) — homepage, one archive, one detail, one taxonomy route, and a 404. A red suite blocks merge to `main`. See SUG-240.
