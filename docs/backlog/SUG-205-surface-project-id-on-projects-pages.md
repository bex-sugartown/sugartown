...
This surfaced during an alignment-audit session comparing `docs/briefs/` PRDs against the live `project` taxonomy — several PRDs cross-reference a project by its `PROJ-XXX` ID (e.g. `monorepo-prd.md` → PROJ-005), but a reader following that reference to the live site has no way to confirm which project is which by ID.
