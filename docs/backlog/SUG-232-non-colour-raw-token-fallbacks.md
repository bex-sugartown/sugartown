...
- **Scope decided 2026-09-19 (Bex): all 144.** The 32 above plus 112 in app-level CSS (`apps/web/src/components/**`, `pages/**`; `PageSections.module.css` alone has 51). The rule in `.claude/rules/tokens.md` covers DS component CSS only, so its scope sentence widens in the same work, through the Instruction & Rule File Write Gate. Size moves from small to medium.
