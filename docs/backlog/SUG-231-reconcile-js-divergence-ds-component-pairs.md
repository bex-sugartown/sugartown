...
- [x] Fix FilterBar: restore the filter header, title, and clear-all button in the web copy; wire `onClearAll` and remove the lint suppression — shipped in Phase 1 (`43dcf0f0`). Ticked 2026-07-22 during close-out: Phase 1 landed the code but never updated this doc. Re-verified against the file rather than the commit message — the `eslint-disable-next-line no-unused-vars` is gone and `onClearAll` is wired to the button's `onClick` — layer: frontend
- [x] Fix CodeBlock: import the Prism line-numbers plugin in the web copy so `showLineNumbers` renders row markup — shipped in Phase 1 (`43dcf0f0`). Ticked 2026-07-22 during close-out, same reason. Re-verified: `import 'prismjs/plugins/line-numbers/prism-line-numbers'` is present and the `line-numbers` global class is applied to `<pre>` when `showLineNumbers` is set — layer: frontend
...
- [x] Define `.wide` in Table's CSS, or remove the dead reference — **removed**, both copies, 2026-07-22 (`3f13daf5`). `.wide` occurs 0 times in either stylesheet (verified directly); CSS modules resolved it to `undefined` and the adjacent `.filter(Boolean)` dropped it, so removal is a provable no-op. Removed rather than defined because `variant="wide"` is already fully implemented by `.wrapWide` on `TableWrap` — writing a `.wide` rule would invent an unreviewed visual design for a live, content-selectable variant (a Phase 0 question, not a cleanup). **This scope item belonged to no phase** — it sat outside Phases 1/1b/2/3 and would have shipped unnoticed — layer: design-system
...
- [x] FilterBar's clear-all button appears and works on a live archive page with active filters; the `eslint-disable` on `onClearAll` is gone — verified 2026-07-22 on `/articles`: activating the author facet set `?author=bex`, "Clear all" then rendered in the FilterBar, and clicking it returned the URL to empty. The `eslint-disable-next-line no-unused-vars` is gone and `onClearAll` is wired at both call sites in `ArchivePage.jsx`
- [x] CodeBlock with `showLineNumbers` renders visible line numbers in the web app, verified in the browser — verified 2026-07-22: `<pre>` carries the `line-numbers` class, the Prism plugin injects `.line-numbers-rows` with 5 row markers for a 5-line sample, 43px wide and visible. **Note the AC was not verifiable as written:** no app caller passes `showLineNumbers` (the PT serializer at `portableTextComponents.jsx:124` passes only `code`/`language`/`filename`), and the only CodeBlock story lives in the DS package, so the *web* copy's fix had no browser surface. Verified via a temporary scratch story, deleted afterwards
