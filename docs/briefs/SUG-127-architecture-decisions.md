...
- Type declarations: esbuild does not emit `.d.ts`, so the build now runs `tsc --emitDeclarationOnly` after the esbuild step. This was originally deferred (DTS was blocked by a lucide-react / React 19 `@types` skew), but the first Vercel deploy failed on it — a clean clone has no `dist/`, so with no emitted declarations the POC's type-check could not resolve DS exports (`has no exported member 'Card'`). The local stale `d.ts` had masked it. Resolved by aligning the DS `@types/react`/`@types/react-dom` to v19 (matching the workspace — the dual-version skew was the root cause) and re-enabling declaration emit. Finding: a build that skips `.d.ts` passes locally on a warm tree but breaks any clean-clone consumer.
