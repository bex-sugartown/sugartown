...
| CLOSE-OUT box, "merge ≠ shipped" | CLAUDE.md §"Issue Done = code on main": explicit verification command given (`git branch --contains <sha> | grep main`) and the rule "A branch that exists at origin/<feat-branch> but is not merged into main is not shipped." Read directly | convention |
