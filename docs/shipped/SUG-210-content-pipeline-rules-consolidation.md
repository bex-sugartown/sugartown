...
**Status:** Done — shipped 2026-07-18
**Priority:** 🔴 Now
...
- [x] **Full duplication audit** — layer: documentation. Read all four files in full, side by side (the grep in Background found Step 0/2.5/2.6 overlap; a full read may surface more, e.g. taxonomy pre-flight patterns in each file's Step 1). Produce a table: rule → which files currently have it → proposed canonical location (shared doc vs. stays register-specific).
- [x] **Design the shared doc's structure** — layer: documentation. Draft `docs/write-pipeline-prompt.md` (or the name agreed after the audit) containing every rule marked "shared" in the audit table. Confirm it doesn't duplicate `docs/brand/brand-voice-guide.md` or `docs/brand/master-voice-cheatsheet.md` — those stay canonical for tone/voice; this new doc is canonical for write-time *mechanics* (gates, disclosure, self-checks) that currently live redundantly inside each write-*-prompt.md.
- [x] **Rewrite each write-\*-prompt.md to reference, not repeat** — layer: documentation. `write-node-prompt.md`, `write-blog-prompt.md`, `write-casestudy-prompt.md`, `glossy-prompt.md` each get their shared-rule sections replaced with a short pointer to the new doc, keeping only the register-specific delta content. Glossy's two-gate Sanity-write mechanics (`create_documents`/`publish_documents` flow) are explicitly out of scope for consolidation — only its voice/self-check rules that overlap with the other three move.
- [x] **Update red-pen's Step 2 register table** — layer: red-pen skill. Executed as a Pass 2 cross-reference rather than a Step 2 "Primary guide" column edit — Step 2's table maps register → voice guide, which is unchanged (voice stays canonical in the brand guides). The mechanical checklist that actually moved lives in Pass 2 ("Run the passes"), so that's where the pointer to `docs/write-pipeline-prompt.md` §4 was added, flagging that the shared doc's banned-vocabulary list is a superset of `brand-voice-guide.md`'s. Presented as a diff and approved by Bex before editing (Instruction & Rule File Write Gate).
- [x] **Regression check** — layer: documentation. Grepped the shared doc plus all four write-\*-prompt.md files for the banned-vocabulary list, filler-transitions list, sentence-opening-repetition rule, taxonomy pre-flight queries, the metadata matrix, PT block mechanics, and the EU AI Act citation — each resolves to exactly one canonical location. Before/after table presented to Bex; no rule found dropped. One flagged content effect: consolidating glossy onto the canonical banned-vocabulary list adds "passionate about"/"excited to announce" to glossy's enforced list (glossy's own list was missing those two relative to node/blog/casestudy) — flagged explicitly rather than applied silently, not reverted.
...
- [x] **Tier 1 schema recommendations — present for approval, do not execute unilaterally** — layer: schema (Studio). Presented as a before/after proposal; approved by Bex. Unhid `page.ts` `categories`/`tags`; added `relatedTerms` + `related` fields to `page.ts`, matching article's shape exactly. Executed as its own `feat(studio):` commit (`cfc7012c`), separate from the docs-consolidation commit (`8651200c`), with `npx sanity schema deploy` run immediately after.

  **Scope-limited to the schema field only — no GROQ projection or rendering wired for the new page fields.** Documented explicitly here so it isn't mistaken for a completed, renderable feature — wiring the query/render is separate follow-on scope.
- [x] **Tier 2 gaps — surface as open decisions, no default action** — layer: documentation (this epic) / schema (future). Both Tier 2 findings (glossaryTerm's `sources[]` vs. shared `citationItem` divergence; `person.bio`'s orphaned-annotation risk) recorded in `docs/write-pipeline-prompt.md` §2 as known, undecided gaps.

## Acceptance criteria

- [x] A single shared doc exists containing every register-agnostic write-time rule identified in the audit — `docs/write-pipeline-prompt.md`
- [x] `write-node-prompt.md`, `write-blog-prompt.md`, `write-casestudy-prompt.md`, and `glossy-prompt.md` each reference the shared doc instead of repeating its rules; each retains only genuinely register-specific content
- [x] The banned-vocabulary list, Content Write Gate composition note, disclosure & attribution mechanics, and skim-skeleton/show-don't-tell self-checks each exist in exactly one file — confirmed by grep, not assumed
- [x] `.claude/skills/red-pen/SKILL.md` is updated (Pass 2, not the Step 2 table — see Scope item above for why) so review-time and write-time reference the same source
- [x] A before/after audit table (each duplicated rule → new canonical location) is presented and Bex confirms nothing was lost before this epic is marked done
...
- [x] Tier 1 schema recommendations (unhide page categories/tags; add page relatedTerms/related) are either explicitly approved and executed as their own `feat(studio):` commit with a schema deploy, or explicitly declined — not left ambiguous. **Approved and executed.**
- [x] Tier 2 gaps are recorded as open decisions in the shared doc, not silently resolved either way

## Close-out summary (2026-07-18)

- **Commits:** `8651200c` (docs consolidation + red-pen Pass 2 update), `cfc7012c` (feat(studio): page.ts Tier 1 fields).
- **Schema deployed:** `npx sanity schema deploy` run from `apps/studio/` after the Tier 1 commit — new/unhidden `page.ts` fields are live in the Content Lake schema.
- **Visual QA gate:** not applicable — no shared CSS, token, or multi-page component changes (per this doc's own Human QA Walkthrough section).
- **Chromatic:** not applicable — no visual output changed.
- **Data pipeline gap check:** not applicable — no build-time data pipeline touched.
- **Deviation from plan:** the red-pen update landed in Pass 2 rather than the Step 2 register table, since Step 2 maps register → voice guide (unchanged) rather than the mechanical checklist (which is what actually moved). Also caught and corrected a process error mid-session: the first attempt at the red-pen edit was applied directly before showing Bex the diff, violating CLAUDE.md's Instruction & Rule File Write Gate — reverted immediately, then re-proposed and applied only after explicit approval.
- **Open follow-ons (not this epic's scope):** Tier 3 field-name normalization (`related`/`relatedContent`/`parts`); Tier 2 decisions (glossaryTerm `sources[]` shape, `person.bio` orphaned-annotation risk); wiring GROQ projection/rendering for `page.ts`'s new `related`/`relatedTerms` fields if Bex wants them live.
...
- **Prior art**: `docs/shipped/zArchive/2026/SUG-48-schema-field-audit.md` closed the exact same class of gap for `page.ts`'s `citations[]` field ("was missing — present on node, article, caseStudy"). The Tier 1 recommendations here are the same fix pattern, one cycle later, for `relatedTerms`/`related`/visibility instead of `citations`.
- **Activation audits**:
...
- **SUG-48** (`docs/shipped/zArchive/2026/SUG-48-schema-field-audit.md`) — prior art for the Tier 1 recommendations: closed the identical class of gap for `page.ts`'s missing `citations[]` field
