...
**GitHub Issue:** [#82](https://github.com/bex-sugartown/sugartown/issues/82) (legacy ID SUG-251; Linear retired 2026-09-05)
...
- **The race is still there.** `RootPage.jsx:60` returns "Loading…" with no SeoHead until its fetch resolves. `SeoHead.jsx:74-161` adds meta and JSON-LD in a `useEffect`. No commits to SeoHead, RootPage, `jsonLd.js` or `seo.js` since this doc was written.
- **Half the build-time option already exists.** SUG-183 (shipped 2026-06-18) added `apps/web/scripts/prerender-content.mjs`, which writes static HTML for article, node and case-study routes with title, description, canonical and OG tags. It writes **no JSON-LD**, and it does not cover `page` routes (`/about`, `/services`, `/contact`, `/platform`), person or glossary-term routes. Extending that script is now the first candidate.
- **The SPA catch-all lives in `apps/web/public/_redirects`** (generated by `build-redirects.js`), not `netlify.toml`.
- **RootPage serves only the `/platform` index**, not every `platform/*` sub-page (`App.jsx:155-165`).
- **Also affected:** `/glossary/:slug` (GlossaryTermPage sends its own JSON-LD client-side) and `/series/:slug`.
- **Risks:** `jsonLd.js` and `seo.js` import `./routes` with no extension, so plain Node cannot import them without a small refactor. Copying their logic into the prerender script would create a second source. SeoHead always appends a new JSON-LD block, so it would duplicate a prerendered one.
- **Related, filed 2026-09-19:** prerendered pages 301 to a trailing-slash URL while their canonical tag omits the slash, and they load no Google Analytics. Filed as #127 (redirect) and #128 (GA), both in the same prerender script.
- **Blocks #83** (SUG-252), recorded as a GitHub relation.
...
SSR, confirmed via the SPA catch-all in `apps/web/public/_redirects`). Every page's SEO meta and JSON-LD are
...
- **GitHub:** [#82](https://github.com/bex-sugartown/sugartown/issues/82)
